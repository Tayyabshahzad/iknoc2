...
class HomeController extends Controller
{
    ...
    public function common_connection(Request $request)
    {
        ...
        $authUserConnections = Connection::where('status','approve')->where('requested_from',$request->auth_id)->get();
        $otherUserConnections = Connection::where('status','approve')->where('requested_to',$request->common_id)->get();
        $arrayAuthUser = array();
        $arrayOtherUser = array();
        foreach($authUserConnections as $authUserConnection){
         
          array_push($arrayAuthUser, $authUserConnection->requested_from);

        }
      
        foreach($otherUserConnections as $otherUserConnection){
            array_push($arrayOtherUser,$otherUserConnection->requested_to);
        }

        $merged_common = (array_merge($arrayOtherUser,$arrayAuthUser));
        $result =   array_unique($merged_common);
            $details =  '';
         for($i=0;$i<count($result); $i++){
            $user = User::find($result[$i]);
            $details.= '<tr>
            <td> '.$user->name.' </td>
            <td> '.$user->email.' </td>
              </tr>';
